...
5. 清除站点 IndexedDB 后首次打开，应显示列表和详情骨架；再次打开应先显示上次缓存，并在后台无感校验。
6. 目录超过 20 条时，首屏 DOM 只应包含首批卡片；滚动接近列表底部后应每次追加 20 条，搜索仍能找到尚未渲染的皮肤。
7. 点击「停用」，确认恢复 DSH 默认外观，但详情仍显示「已安装」。
8. 再次使用后切换到另一个皮肤，确认前一个皮肤自动停用。
9. 卸载正在使用的皮肤，确认先恢复默认外观再删除 package。
10. 制造断网后重试安装，确认错误可见且 profile `package.json` 不残留幽灵依赖。
