...
import { companionAsSkin } from './install-resolution.js';
import { ensureSkinRegistration, manifestFile, marketStateFile, profilePatchFile, readDependencies, readMarketState, restoreFile, snapshotFile, writeMarketState, } from './profile.js';
    ...
    try {
        ...
        for (const skin of catalog) {
            for (const companion of skin.install.companions ?? []) {
                ...
                if (seenCompanions.has(companion.package) || dependencies[companion.package] === undefined || state.managedCompanions?.[companion.package] === undefined)
                    continue;
