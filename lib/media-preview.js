...
export function parseGeneratedMediaManifest(value) {
    if (typeof value !== 'object' || value === null || Array.isArray(value))
        return undefined;
    const entries = Object.entries(value);
    if (!entries.every(([source, digest]) => /^https?:\/\//i.test(source) && typeof digest === 'string'))
        return undefined;
...
export function generatedMediaManifestUrl() {
...
export function previewSourceCandidates(source, fallbacks = []) {
...
export function generatedMediaFor(entry, source, kind) {
    if (!generatedMediaEnabled() || source === undefined || entry.media === undefined)
        return undefined;
    if (localGeneratedMediaSources !== undefined && !localGeneratedMediaSources.has(source))
        return undefined;
    if ((kind === 'list' || kind === 'avatar' || kind === 'recommendation' || kind === 'card') && source === getCatalogListScreenshot(entry))
