...
import { companionNeedsInstall, ensureBuildAllowed, ensureSkinRegistration, installedSpecMatches, cleanupCompatibilityPatchFiles, detachCompatibilityPatches, pnpmLockfile, pnpmWorkspaceFile, profilePatchFile, readDependencies, readMarketState, readProfileBundles, assertNoLoaderConflicts, InstallConflictError, compatibilityPatchFile, marketStateFile, packageManifest, patchedDependenciesNeedSync, removeSkinRegistration, restoreFile, restoreManifest, runtimeState, snapshotFile, validateInstalledSkin, writeMarketState, } from './profile.js';
...
export class SkinLifecycle {
    ...
    async installCompanions(skin, operation) {
        ...
        for (const companion of skin.install.companions ?? []) {
            ...
            ensureSkinRegistration(this.options.profileDir, companionAsSkin(skin, companion), !this.companionOwnersEnabled(companion.package, enabled));
        }
    }
    companionOwnersEnabled(packageName, enabled) {
        return this.catalog.some(item => enabled.has(item.id) && (item.install.companions ?? []).some(companion => companion.package === packageName));
    }
    async syncInstalledCompanions(state, live) {
        ...
        for (const item of this.catalog) {
            for (const companion of item.install.companions ?? []) {
                if (seen.has(companion.package) || dependencies[companion.package] === undefined)
                    continue;
    ...
    companionStillNeeded(packageName, exceptSkinId) {
        return this.catalog.some(item => {
            if (item.id === exceptSkinId)
                return false;
            if (!(item.install.companions ?? []).some(companion => companion.package === packageName))
                return false;
            return readDependencies(this.options.profileDir)[item.package] !== undefined;
        });
    }
    async uninstallUnusedCompanions(skin, operation) {
        for (const companion of skin.install.companions ?? []) {
            if (this.companionStillNeeded(companion.package, skin.id))
                continue;
            if (readDependencies(this.options.profileDir)[companion.package] === undefined)
                continue;
