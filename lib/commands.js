...
const runCommand = (file, args, options) => new Promise(resolvePromise => {
    ...
    const abort = () => { aborted = true; kill('SIGTERM'); };
...
export function createPnpmProvisioner(execute = runCommand) {
    ...
    return async (options) => {
        ...
        ready = (async () => {
            ...
            const probe = async () => execute('pnpm', ['--version'], { signal: options?.signal, env });
