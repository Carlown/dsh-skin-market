...
export function apply(ctx, config) {
    ctx.inject(['webServer', 'loader', 'agents'], hostContext => {
        ...
        hostContext.inject(['desktopPnpm'], desktopContext => {
            ...
            desktopHost.effect(() => mountRoutes(host, { profile: current.name, profileDir: current.dir, runner: desktopRunner(service, current.dir), hostKind: 'desktop' }), 'dsh-skin-market: desktop routes');
        });
    });
}
