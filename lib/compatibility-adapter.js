...
import { hasRuntimeCapability, KEYED_SLOT_CAPABILITY_PREFIX } from './runtime.js';
...
function applicableAdapters(skin, runtime, includeBuiltIns) {
    ...
    const adapters = includeBuiltIns
        ...
        : declared;
    return adapters.filter(adapter => satisfiesVersionRange(runtime.version, adapter.when)
        && hasRuntimeCapability(runtime, `${KEYED_SLOT_CAPABILITY_PREFIX}${adapter.slot}`));
}
export function assessCompatibility(skin, runtime) {
    if (runtime.version === null)
        return {
            decision: 'unknown',
    ...
    if (skin.compatibility.dsh === 'unverified')
        return {
            decision: 'unknown',
    ...
    if (satisfiesVersionRange(runtime.version, skin.compatibility.dsh))
        return {
            decision: 'compatible',
