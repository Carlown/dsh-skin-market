...
function targetDirectory(profileDir: string, args: readonly string[]): string {
 ...
 */
export function pluginArgsFor(profileDir: string, args: readonly string[]): string[] {
  const commandIndex = args.findIndex(arg => arg === 'add' || arg === 'remove')
  if (commandIndex < 0 || args.includes('-w') || args.includes('--workspace-root')) return [...args]
  if (!existsSync(join(targetDirectory(profileDir, args), 'pnpm-workspace.yaml'))) return [...args]
