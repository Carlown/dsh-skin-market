      ...
      <main className={css.detail}>
        {loading ? <div className={css.loading} role="status"><IconLoadingOutline16 size={16} />正在加载皮肤详情…</div> : selected !== undefined && state !== null ? <>
          ...
          <div className={css.aboutGrid}>
            <article><h3>关于此皮肤</h3><p>{selected.description}</p><div className={css.tags}>{selected.tags.map(tag => <Pill className={css.staticPill} key={tag}>{tag}</Pill>)}</div><dl className={css.metadata}><div><dt>许可证</dt><dd>{selected.license.code}</dd></div><div><dt>代码商业使用</dt><dd>{selected.license.commercialUse ? '许可证允许' : '未获授权'}</dd></div><div><dt>模式</dt><dd>{selected.modes.join(' / ')}</dd></div></dl>{compatibilityUnverified && !manualOnly && <p className={css.notice}>维护者尚未声明 DSH 兼容范围。市场可以自动安装并保持停用；使用前请自行确认与当前 DSH 版本兼容。</p>}{manualOnly && <p className={css.notice}>该仓库缺少市场自动注册所需的信息；请前往 GitHub 按维护者说明手动安装。</p>}{selected.review?.preview === 'repository-card' && <p className={css.notice}>该仓库没有可识别的皮肤截图，当前展示的是 GitHub 仓库卡片，并非界面预览。</p>}{selected.license.notice && <p className={css.notice}>{selected.license.notice}</p>}</article>
